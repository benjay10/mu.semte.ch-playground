...
*	semtech/mu-login-service
*	semtech/mu-dispatcher
*	semtech/mu-identifier
*	semtech/mu-delta-notifier
*	semtech/mu-javascript-template (to make my own mock-access)

I also made a Docker container for serving the Ember app (development with live reload), and put this behind the mu-dispatcher so everything is nicely contained.
...
The following are the defined classes (`owl:Class`) in the ontology and will certainly be used in the webapp:
...
The following classes are defined in the ontology using OWL, but are not used elsewere. This is because you need some form of reasoning to be able to find class definitions and superclass definitions on individuals that use OWL constraints.

*	**BigAuction** (an auction with at least 5 lots (e.g.))
*	**AbsoluteExpensiveAutionLot** (a lot that starts with a high price)
*	**RelativeExpensiveAuctionLot** (a lot that has come up to a high prices due to excessive bidding)
...
There are a few things that I would like to make note of with relation to Ember data models.
...
Virtuoso database has a few quirks. It does not allow for using blank nodes in certain queries, but allows it in similar, but slightly different queries. This is annoying when you used a tool like Protégé which creates blank nodes when defining constraints. Manually writing this out to use dummy nodes solves the issue, or using a newer version of Virtuoso (whenever it comes out in the future) could solve that problem too.  
Some `INSERT` queries fail because of some internal Virtuoso bug. It sometimes complains about a function called `string_output_string`. This can be solved by applying a bogus `BIND` in the `WHERE` clause, like so: `BIND(?s as ?s)`. This is stupid.
...
*	mu-authorization: `INSPECT_OUTGOING_SPARQL_QUERY_RESPONSES` log does not do the expected. The query is logged instead of the responses.
